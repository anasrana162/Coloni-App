...
const AddVisitHomeScreenVig: React.FC<{
  route: {
    params?: {
      edit: boolean;
      index: number;
  ...
  route: {
    params: {edit, index, id, item, screenName, residentId} = {
      edit: false,
      index: -1,
      id: '',
      screenName: '',
  ...
  const [marWork, setmarWork] = useState('');
  const [enterNote, setEnterNote] = useState('');
  const [VisiorName, setVisiorName] = useState('');
  const [date, setDate] = useState(new Date());
  const [images, setImages] = useState('');
  const [Visit, setVisit] = useState('');
  // const [marWork, setMarkW] = useState('');
  //const [date, setDate] = useState(new Date());
  // const [images, setImages] = useState('');
  // const [Visit, setVisit] = useState('');
  ...
  // });
  useEffect(() => {
    const residentInfo = `${item?.resident?.street?.name || ''} ${
      item?.resident?.home || ''
    }`;
    setResident(residentInfo);
    setVisit(item?.visitorName);
    const visitDate = item?.createdAt;
    setDate(visitDate);
    setmarWork(item?.marWork);
    setEnterNote(item?.enterNote);
    setImages(item?.images);
  }, []);
  const handleSubmit = async () => {
    const visitType =
      screenName === 'frequent' ? 'FrequentVisit' : 'EventualVisit';
    const payload = {
      markWork: marWork,
      images: images,
      enterNote: enterNote,
    ...
    const result = await DismissVisitServices.createEntry(payload);
      ...
      {fetching ? (
        ...
        <ScrollView
          ...
          <View style={styles.container}>
            <Text style={{...styles.topText, color: colors.primary}}>
              🏠{Resident}
            </Text>
            <Text style={{...styles.topText, color: colors.gray1}}>
              {Visit}
            </Text>
            <Text style={{...styles.topText, color: colors.primary}}>
              {trans('Expected Date')}
          ...
          <LabelInput
            label={trans('Mar work#')}
            placeholder={trans('Mar work#')}
            defaultValue={marWork}
            onChangeText={value => setmarWork(value)}
          />
          <MultiLineInput
            label={trans('Entry Note')}
            placeholder={trans('Entry Note')}
            defaultValue={enterNote}
            onChangeText={value => setEnterNote(value)}
          />
          <UploadImage
            defaultValue={images}
...
export default AddVisitHomeScreenVig;
