        ...
        <View style={styles.center}>
          ...
          <Text
            style={[
              ...
              {marginTop: rs(10), color: colors.primary},
            ]}>
            {trans('Image Upload')}
          </Text>
        </View>
...
const AddOutstandingCharges: React.FC<{
  route: {params?: {id?: any; ResidentName?: string}};
}> = ({
  route: {
    params: {id, ResidentName} = {
      id: '',
      ResidentName: '',
    },
  },
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...values.current,
      paymentType: values?.current?.paymentType?.name,
    };
    console.log('payloads.....', payload);
    if (checkEmptyValues(payload, 'files')) {
      loading.current = true;

      const result = await outstandingBalancesService.create(payload);
      const {status, body, message} = result as apiResponse;
      if (status) {
        console.log('Body from outsanding charges:', result);
...
export default AddOutstandingCharges;
