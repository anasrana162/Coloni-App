...
const AddPayment: React.FC<{onChange: (value: any) => void}> = ({onChange}) => {
  ...
  const handlePress = async () => {
    if (value.current) {
      setLoading(true);
      const result = await paymentService.createType({name: value.current});
      const {body, status,message} = result as apiResponse;
      console.log("checking api response--payment type",result);
      if (status) {
        onChange(body);
