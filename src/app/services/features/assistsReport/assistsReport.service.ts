...
import { assistsReportEndPoint } from '../endpoint.api';
import { rootApi } from '../rootApi';
import { Alert, Platform } from 'react-native';
import RNFS from 'react-native-fs';

class assistReportServices {
  async fetch(date: string, trans: any) {

    const url = `${assistsReportEndPoint.fetch}?date=${date}`;
    ...
    try {
      ...
      const path =
        Platform.OS == 'ios'
          ...
          }/Assists_Report_${new Date().getMilliseconds()}.csv`
...
export default new assistReportServices();
