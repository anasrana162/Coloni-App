...
import { annualIncomeEndPoint } from '../endpoint.api';
import { rootApi } from '../rootApi';
import { Alert, Platform } from 'react-native';
import RNFS from 'react-native-fs';

class annualIncomeServices {
  async fetch(values: any, trans: any) {

    if (!values?.reportType)
      return Alert.alert('Report Type Missing!', 'Please select Report Type');
    if (!values?.paymentType)
      return Alert.alert('Payment Type Missing!', 'Please select Payment Type');

    const url = `${annualIncomeEndPoint.fetch}?paymentType=${values?.paymentType}&reportType=${values.reportType}&payDay=${values.payDay}`;
    ...
    try {
      ...
      const path =
        Platform.OS == 'ios'
          ...
          }/Annual_Income_${new Date().getMilliseconds()}.csv`
...
export default new annualIncomeServices();
