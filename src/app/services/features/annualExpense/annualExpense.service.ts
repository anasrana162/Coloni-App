...
import { annualExpenseEndPoint } from '../endpoint.api';
import { rootApi } from '../rootApi';
import { Alert, Platform } from 'react-native';

import RNFS from 'react-native-fs';
class annualExpenseServices {
  async fetch(values: any, trans: any) {
    if (!values?.reportType)
      return Alert.alert('Report Type Missing!', 'Please select Report Type');

    const url = `${annualExpenseEndPoint.fetch}?date=${values?.date}&reportType=${values.reportType}`;
    ...
    try {
      ...
      const path =
        Platform.OS == 'ios'
          ...
          }/Annual_Expense_${new Date().getMilliseconds()}.csv`
...
export default new annualExpenseServices();
