...
import { incidentsReport } from '../endpoint.api';
import { rootApi } from '../rootApi';
import { Alert, Platform } from 'react-native';
import RNFS from 'react-native-fs';


class incidentsReportServices {
  async fetch(date: string, trans: any) {

    const url = `${incidentsReport.fetch}?payDay=${date}`;
    ...
    try {
      ...
      const path =
        Platform.OS == 'ios'
          ...
          }/Incidents_Report_${new Date().getMilliseconds()}.csv`
...
export default new incidentsReportServices();
