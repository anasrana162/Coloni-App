...
import { cancelChargesEndPoint } from '../endpoint.api';
import { rootApi } from '../rootApi';
import { Alert, Platform } from 'react-native';
import RNFS from 'react-native-fs';

class cancelChargesServices {
  async fetch(date: string, trans: any) {

    const url = `${cancelChargesEndPoint.fetch}?date=${date}`;
    ...
    try {
      ...
      const path =
        Platform.OS == 'ios'
          ...
          }/Cancel_Charges_${new Date().getMilliseconds()}.csv`
...
export default new cancelChargesServices();
