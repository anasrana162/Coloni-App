...
import { advancedSFEndPoint } from '../endpoint.api';
import { rootApi } from '../rootApi';
import { Alert, Platform } from 'react-native';

import RNFS from 'react-native-fs';
class advancedSFServices {
  async fetch(values: any, trans: any) {

    const url = `${advancedSFEndPoint.fetch}`;
    console.log('URL AdvancedSF:', url);
    ...
    try {
      ...
      const path =
        Platform.OS == 'ios'
          ...
          }/Advance_SF_${new Date().getMilliseconds()}.csv`
...
export default new advancedSFServices();
