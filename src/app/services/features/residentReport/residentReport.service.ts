...
import { residentReportEndPoint } from '../endpoint.api';
import { rootApi } from '../rootApi';
import { Alert, Platform } from 'react-native';
import RNFS from 'react-native-fs';

class residentReportServices {
  async fetch(date: string, trans: any) {
    const url = `${residentReportEndPoint.fetch}?date=${date}`;
    ...
    try {
      ...
      const path =
        Platform.OS == 'ios'
          ...
          }/Resident_Report_${new Date().getMilliseconds()}.csv`
...
export default new residentReportServices();
